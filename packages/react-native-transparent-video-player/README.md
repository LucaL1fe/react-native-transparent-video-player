...
Renders via Skia on iOS and a native ExoPlayer + OpenGL view on Android (v0.3.0+) — no `minSdkVersion` override needed. Android needs `expo-modules-core` (built into every Expo app; bare RN: `npx install-expo-modules`) and a development-build rebuild after install (not Expo Go). Previously published as `react-native-transparent-video-skia`.
